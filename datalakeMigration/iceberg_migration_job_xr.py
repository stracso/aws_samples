...
4. Rewriting data files to update internal path references
...
class IcebergMigrationJob:
    ...
    def _register_tables(self):
        """Drop existing tables in destination and re-register them from the new metadata location."""
        print("Registering tables at new location in destination catalog...")
        ...
        for table_name in self.table_names:
            ...
            # The desired table root location in the destination bucket
            dest_table_location = f"s3://{self.dedicated_uri}/{table_name}"

            fqn = f"glue_catalog.{self.database}.{table_name}"

            print(f"{dryrun_prefix}Dropping table {fqn} (if exists) in {self.dest_region}...")
            if not self.dryrun:
            ...
            # The metadata.json still contains the source location internally.
            # Update the table location to point to the destination bucket.
            print(f"{dryrun_prefix}Setting table location to {dest_table_location}")
            if not self.dryrun:
                self.spark.sql(
                    f"ALTER TABLE {fqn} SET LOCATION '{dest_table_location}'"
                )

            print(f"Registered: {fqn}")

    # ------------------------------------------------------------------
    # Step 3: Rewrite data files to update internal path references
    # ------------------------------------------------------------------
